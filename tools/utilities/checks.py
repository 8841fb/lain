...
def has_permissions(**permissions):
    ...
    async def predicate(ctx: commands.Context):
        if isinstance(ctx, int):
            return [
                permission for permission, value in permissions.items() if value == True
            ]
...
def donator(booster: bool = False):
    ...
    async def predicate(ctx: commands.Context):
        guild = ctx.bot.get_guild(1119909593234034690)
        role = guild.get_role(1138276637230760017)
        ...
        if booster:
            ...
            if not user or not user.premium_since:
                raise commands.CommandError(
                    f"You must **boost** the lain [**Discord Server**](https://discord.gg/opp) to use `{ctx.command.qualified_name}`"
                )
        ...
        if not user or not donator_check:
            raise commands.CommandError(
                f"You must be a **donator** to use `{ctx.command.qualified_name}` - [**Discord Server**](https://discord.gg/opp)"
            )
